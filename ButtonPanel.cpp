...
void ButtonPanel::init()
{
    ...
    wxSizer *max_round_sizer = new wxBoxSizer(wxHORIZONTAL);

    wxSpinCtrl *max_rounds = new wxSpinCtrl(max_round_panel);
    max_rounds->SetValue(max_round_count);
    max_rounds->Bind(wxEVT_COMMAND_SPINCTRL_UPDATED, &ButtonPanel::updateMaxRounds, this);
    wxStaticText *max_round_text = new wxStaticText(max_round_panel, wxID_ANY, "Max Rounds:");
    max_round_sizer->Add(max_round_text, 0, wxALIGN_LEFT, 0);
    max_round_sizer->Add(max_rounds, 0, 0, 0);
...
void ButtonPanel::updateMaxRounds(wxSpinEvent& event)
{
    setMaxRoundCount(event.GetPosition());
}


void ButtonPanel::setMaxRoundCount(const int round)
{
    max_round_count = round;
}